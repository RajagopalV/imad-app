...
app.get('/testDb', function (req, res) {
    ...
    pool.query("SELECT article.title,article.heading,article.date,article.content,comment.comment FROM article INNER JOIN comment ON comment.article_id = article.id WHERE article.title = 'article1'",function(err,result){
        if(err){
            res.status(500).send(err.toString());
        }else{
            res.send(JSON.stringify(result.rows));
    ...
    pool.query("SELECT article.id,article.title,article.heading,article.date,article.content,comment.comment FROM article JOIN comment ON comment.article_id = article.id WHERE article.title ='"+ req.params.articleName +"'",function(err,result){
        if(err){
            ...
            if(result.rows.length === 0){
                ...
                if(req.session && re.session.auth){    
