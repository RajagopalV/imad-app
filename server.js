...
app.get('/testDb', function (req, res) {
    ...
    pool.query("SELECT article.title,article.heading,article.date,article.content,comment.comment FROM article WHERE title='article1' JOIN comment ON article.id=comment.artice_id",function(err,result){
        if(err){
            res.status(500).send(err.toString());
        }else{
            res.send(JSON.stringify(result.rows));
